...
use crate::Square;
use crate::{structures::rooms::RoomSquare, Area, Map, MapInterface, MapType, Point, Tile};

#[derive(Clone)]
pub struct MapSquareRooms {
    pub map: Map,
    min_room_width: i64,
    max_room_width: i64,
    min_room_height: i64,
    max_room_height: i64,
    list_of_rooms: Vec<RoomSquare>,
}

impl MapInterface for MapSquareRooms {
    // Gets
...
impl MapSquareRooms {
    pub fn new(w: i64, h: i64) -> Self {
        let mut new_map = MapSquareRooms {
    ...
    fn set_room_limits(&mut self) {
        ...
        // let max_mod = 8.0;
        // let min = 4;
        // self.min_room_width = (((width * height) as f64).sqrt() / min_mod) as i64;
        // self.max_room_width = (((width * height) as f64).sqrt() / max_mod) as i64;
        // self.min_room_height = (((width * height) as f64).sqrt() / min_mod) as i64;
        // self.max_room_height = (((width * height) as f64).sqrt() / max_mod) as i64;
        self.min_room_width = 4;
        self.max_room_width = 10;
        self.min_room_height = 4;
        self.max_room_height = 12;
        // Force minimun room size
        // if self.min_room_width < min {
        //     self.min_room_width = min;
        // }
        // if self.min_room_height < min {
        //     self.min_room_height = min;
        // }
        // if self.max_room_width < self.min_room_width {
        //     self.max_room_width += self.min_room_width;
        // }
        // if self.max_room_height < self.min_room_height {
        //     self.max_room_height += self.min_room_height;
        // }
    }
    fn create_rooms_no_intersect(&mut self) {
        ...
        for _ in 0..1000 {
            ...
            for room in self.list_of_rooms.iter() {
                if possible_location.intersects_square(&room.area) {
                    failed = true;
                    break;
                }
            }
    ...
    fn create_tunnels(&mut self) {
        ...
        for (i, current_room) in self.list_of_rooms.iter().enumerate() {
            ...
            sx = min(current_room.area.x1, previous_center.x);
            ex = max(current_room.area.x1, previous_center.x) + 1;
            sy = min(current_room.area.y1, previous_center.y);
            ey = max(current_room.area.y1, previous_center.y) + 1;

            if rng.gen::<bool>() {
                for x in sx..ex {
                    points.push(Point::new(x, current_room.area.y1));
                }
                for y in sy..ey {
                ...
                for y in sy..ey {
                    points.push(Point::new(current_room.area.x1, y));
                }
                for x in sx..ex {
